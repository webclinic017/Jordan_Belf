...
data00 = pd.read_csv('SQ_1d.csv', index_col = 0, parse_dates = True)
...
class movav_50d_200d_swtich(bt.Strategy):
	...
	def __init__(self):
		...
		for i, d in enumerate(self.datas):
			self.inds[d] = dict()
			self.inds[d]['movav50'] = bt.indicators.SimpleMovingAverage(d.close, period = 5)
			self.inds[d]['movav200'] = bt.indicators.SimpleMovingAverage(d.close, period = 20)
			...
			if not pos:
				if self.inds[d]['movav_cross'][0] == 1.0:
					...
					if self.inds[d]['movav_cross'][0] == 1.0:
						...
						self.order = self.buy(data = d, size = 10, exectype = bt.Order.StopTrail, trailpercent= 0.01)
