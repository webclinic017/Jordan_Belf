...
#RTX
#BA
data_30m = y.download(tickers=ticker, period = "30d", interval="60m")
...
def raw_VBE_upper(percent_change_day,sd,data,mu):
	...
	for i in range(len(data-1)):
		...
		upperband.append(upper_raw_band_f)
	return upperband
...
def raw_VBE_lower(percent_change_day,sd,data,mu):
	...
	for i in range(len(data-1)):
		...
		lowerband.append(lower_raw_band)
	return lowerband
...
def lag(data,raw):
	...
	wma5 = wma(5,raw)
	wma2 = wma(2,raw)
	wma21_pc = percent_change(wma21)
	wma17_pc = percent_change(wma17)
	wma13_pc = percent_change(wma13)
	wma9_pc = percent_change(wma9)
	wma5_pc = percent_change(wma5)
	wma2_pc = percent_change(wma2)
...
def plot_vbe(data):
	...
	plt.plot(data.index.astype(str), data['Close'], label = 'Price', color = 'red')
	plt.plot(data.index.astype(str), data['Upper Band'], label = 'Upper', color = 'blue')
	plt.plot(data.index.astype(str), data['Lower Band'], label = 'Lower', color = 'blue')
	#plt.plot(data.index.astype(str), data['Raw Upper'], label = 'Raw Upper', color = 'green')
	#plt.plot(data.index.astype(str), data['Raw Lower'], label = 'Raw Lower', color = 'green')
...
def VBE_upper(period, data, data_adj_close):
	#STEP 1 
	...
	data['Raw Upper'] = raw_upper

	#STEP 3 /4 
	#SMOOTH USING CWMAs and adjusting for lag
	data['Upper Band'] = lag(wma(period, raw_upper), raw_upper)

	#Return the most recent point in the vbe
	return data['Upper Band']

def VBE_lower(period, data, data_adj_close):
	#STEP 1 
	#CALCULATING THE STANDARD DIV
	#first get the percent change of every day for teh past 21d
	pc21 = percent_change(data_adj_close)
	...
	data['Raw Lower'] = raw_lower

	#STEP 3 /4 
	#SMOOTH USING CWMAs and adjusting for lag
	data['Lower Band'] = lag(wma(period,raw_lower), raw_lower) 
	#Return the most recent point in the vbe
	return data['Lower Band']


#VBE_channel(21,data_30m,data_30m_adj_close)
print(VBE_upper(21, data_30m, data_30m_adj_close))
VBE_lower(21, data_30m, data_30m_adj_close)
plot_vbe(data_30m)
